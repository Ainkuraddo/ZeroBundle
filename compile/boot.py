...
def getWorkDir():
    if getattr(sys, 'frozen', False):
        ...
        if work_dir.startswith("/Application") or work_dir.startswith("/private") or work_dir.startswith(os.path.expanduser("~/Library")):
            ...
            work_dir = os.path.expanduser("~/Library/Application Support/Ainkuraddo")
...
def addSourcePaths(work_dir):
    if sys.platform == "darwin":
        ...
        sys.path.insert(0, os.path.join(work_dir, "core"))  # Ainkuraddo source code
...
def setup():
    ...
    if sys.argv[0].endswith("Resources/boot.py"):
        sys.argv[0] = sys.argv[0].replace("Resources/boot.py", "MacOS/Ainkuraddo")
